function [U, V, Cov, Obj, U_scale_his] = TreeSGS_g(X, gamma, lambda, k, C, maxiter, U_ini, V_ini, group, mx, Vnorm, ratio, pcut, sigma)
%% Gaussian kernal version of treeSGS algorithm
...
for iter = 1:maxiter
    ...
    % solve V
    %%%%%%%%%%
    V = SolveV_SparseGroup(X, V, U, group, mx, Vnorm, ratio, pcut, sigma);
...
%%
function V = SolveV_SparseGroup(X, V, U, group, mx, Vnorm, ratio, pcut, sigma)
iter_max = 100;
k = size(U, 2);
n_nodes = size(mx,1);
...
for iter = 1 : iter_max
    ...
    for kk = 1 : k
        ...
        if tmp ~= 0
            ...
            maxvv = max(vv);
            minvv = min(vv);
            step = (maxvv-minvv)/(binnum-1);

            % calculate entropy 
            % based on Gaussian Kernel with sigma
            etpy = zeros(n_nodes,1);
            etpy_gain = zeros(n_nodes,1);

            for i = 1:n_nodes
                subv = vv(range{i});
                [pp, ~] = ksdensity(subv, minvv:step:maxvv, 'width', (maxvv-minvv)/sigma);
                pp = pp/sum(pp);
                nz_pp = pp;
                nz_pp(nz_pp == 0) = [];
                etpy(i) = -sum(nz_pp.*log(nz_pp))/length(pp);
            end;
